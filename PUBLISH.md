...
### 方法1: Trusted Publishing with OIDC（推奨）

2025年7月に一般提供開始された最新の認証方式です。

✅ **メリット**:
- **トークン管理不要** - NPM_TOKENを保存・管理する必要がない
- **より安全** - 短命なOIDCトークンを使用（ワークフロー実行ごとに生成）
- **自動Provenance** - パッケージの出所証明が自動付与
- **npm公式推奨** - npmとGitHubが推奨するセキュアな方法

❌ **デメリット**:
- 初回は手動公開が必要（Trusted Publisher設定のため）
- GitHub Actions（クラウドホストランナー）必須

**必要条件**:
- npm CLI v11.5.1以降
- GitHub Actions（クラウドランナー）
- パッケージが既に公開済み（Trusted Publisher設定のため）

### 方法2: Granular Access Token

従来のトークンベース認証です。Classic Tokenは廃止され、Granular Access Tokenのみ利用可能です。
...
❌ **デメリット**:
- **トークン管理が必要** - 安全な保管とローテーションが必要
- **セキュリティリスク** - トークン漏洩のリスク
- **定期ローテーション推奨** - 3-6ヶ月ごとのトークン更新が推奨される
- Provenance自動付与なし
